...
```svelte
<script lang="ts">
  import { FlagCn, FlagGb, FlagUs } from '@sankyu/svelte-circle-flags'
  ...
  const Flag = $derived(FLAGS[normalized as keyof typeof FLAGS])
</script>

{#if Flag}
  <Flag width={36} height={36} />
{:else}
  <span title="Unknown country code">{code.toUpperCase()}</span>
{/if}
```
...
```tsx
import { coerceFlagCode, DynamicFlag } from '@sankyu/solid-circle-flags'

export function CountryFlag(props: { codeFromApi: string }) {
  const safeCode = () => coerceFlagCode(props.codeFromApi)
  return <DynamicFlag strict code={safeCode()} width={36} height={36} />
}
```
