...
When country codes are only known at runtime, use dynamic flag rendering.

For flags known at build time, use [named imports](/docs/guides/getting-started/usage/#1-named-imports) for best bundle size and performance.

:::

## Why use dynamic flags?

In many applications, country codes are not known until runtime. Examples include:

- User-selected countries in forms or settings
- Country data from APIs or databases
- Dynamic content based on user location

To handle these scenarios, you need a way to render flags based on ISO country codes at runtime.
