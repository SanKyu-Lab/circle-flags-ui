---
title: Using CDN (No npm)
description: Load @sankyu/react-circle-flags from an ESM CDN without installing dependencies.
---

import { Card, CardGrid } from '@astrojs/starlight/components'

This page is for **prototyping** and **online sandboxes**.

If you control your build, prefer installing from npm and bundling locally (best performance and offline support).

:::caution[Security & stability]

- You are executing JavaScript from a third-party origin. Only use CDNs you trust.
- Always pin versions in production to avoid unexpected breaking changes.
  :::

## Quick start (esm.sh)

```html
<!DOCTYPE html>
<html>
  <head>
    <meta charset="UTF-8" />
    <script type="importmap">
      {
        "imports": {
          "react": "https://esm.sh/react@19.2.7",
          "react-dom/client": "https://esm.sh/react-dom@19.2.7/client"
        }
      }
    </script>
  </head>
  <body>
    <div id="root"></div>

    <script type="module">
      import { createElement } from 'react'
      import { createRoot } from 'react-dom/client'
      import { FlagUs } from 'https://esm.sh/@sankyu/react-circle-flags@1.6.7?exports=FlagUs'

      function App() {
        return createElement(FlagUs, { width: 64, height: 64, title: 'US' })
      }

      createRoot(document.getElementById('root')).render(createElement(App))
    </script>
  </body>
</html>
```

## Import only what you need

Most ESM CDNs support an `exports` query to avoid pulling in unused exports.

```js
import {
  FlagCn,
  FlagUs,
} from 'https://esm.sh/@sankyu/react-circle-flags@1.6.7?exports=FlagCn,FlagUs'
```

## Runtime codes (finite set)

When country codes are runtime values, keep it offline-first by mapping a finite set:

```js
import {
  FlagCn,
  FlagGb,
  FlagUs,
} from 'https://esm.sh/@sankyu/react-circle-flags@1.6.7?exports=FlagCn,FlagGb,FlagUs'
import { createElement } from 'https://esm.sh/react@19.2.7'

const FLAGS = { cn: FlagCn, gb: FlagGb, us: FlagUs }

export function CountryFlag({ code }) {
  const normalized = String(code).trim().toLowerCase()
  const Flag = FLAGS[normalized]
  return Flag
    ? createElement(Flag, { width: 36, height: 36 })
    : createElement('span', null, String(code).toUpperCase())
}
```

## Version pinning

:::tip[Production best practice]
Pin versions in production:
:::

```diff
- import { FlagUs } from 'https://esm.sh/@sankyu/react-circle-flags?exports=FlagUs'
+ import { FlagUs } from 'https://esm.sh/@sankyu/react-circle-flags@1.6.7?exports=FlagUs'
```

## CDN options

All of these provide ESM support with dependency resolution:

<CardGrid>
  <Card title="esm.sh" icon="star">
    Great defaults, good caching, widely used.
  </Card>
  <Card title="Skypack" icon="external">
    Stable ESM CDN with long history.
  </Card>
  <Card title="esm.run" icon="rocket">
    Fast ESM CDN (jsDelivr ecosystem).
  </Card>
</CardGrid>
