...
```svelte
<script lang="ts">
  import { DynamicFlag, isFlagCode } from '@sankyu/svelte-circle-flags'

  let { code }: { code: string } = $props()
  const normalized = $derived(code.trim().toLowerCase())
  const validCode = $derived(isFlagCode(normalized) ? normalized : null)
</script>

{#if validCode}
  <DynamicFlag strict code={validCode} width={32} height={32} />
{:else}
  <span class="text-red-500">{code.toUpperCase()} (invalid)</span>
{/if}
```
...
```svelte
<script lang="ts">
  import { coerceFlagCode, DynamicFlag } from '@sankyu/svelte-circle-flags'

  let { codeFromApi }: { codeFromApi: string } = $props()
  const safeCode = $derived(coerceFlagCode(codeFromApi))
</script>

<DynamicFlag strict code={safeCode} width={32} height={32} />
```
...
<script lang="ts">
  ...
  let { code, fallbackCode = 'un' }: { code: string; fallbackCode?: FlagCode } = $props()
  const safeCode = $derived(coerceFlagCode(code, fallbackCode))
</script>
...
```svelte
<script lang="ts">
  import { isFlagCode } from '@sankyu/svelte-circle-flags'
  import type { FlagCode } from '@sankyu/svelte-circle-flags'
...
<form onsubmit={(event) => { event.preventDefault(); handleSubmit() }}>
  <input bind:value={code} placeholder="Enter country code" />
...
<CardGrid stagger>
  <LinkCard
    ...
    href="/docs/guides/advanced/flag-utils/"
  />
  <LinkCard
    title="Dynamic Flags"
