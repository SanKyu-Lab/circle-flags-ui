...
<script lang="ts">
  ...
  let code = $state('')
  const isValid = $derived(FlagUtils.isValidCountryCode(code))
</script>

<input bind:value={code} placeholder="Enter country code (e.g., us, cn, gb)" />
{#if !isValid && code}
  <p class="text-red-500">Invalid country code</p>
{/if}
```
...
<script lang="ts">
  ...
  import { FlagUtils, FlagUs } from '@sankyu/svelte-circle-flags'

  let size = $state(FlagUtils.sizes.md)
...
<script lang="ts">
  ...
  const componentName = $derived(FlagUtils.getComponentName(code))
  const isValid = $derived(FlagUtils.isValidCountryCode(code))
</script>
...
<CardGrid stagger>
  <LinkCard
    ...
    href="/docs/guides/advanced/type-utilities/"
  />
  <LinkCard
    title="Dynamic Flags"
